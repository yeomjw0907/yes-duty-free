...
- [x] **관리자(Admin) 2단계** – 면세상품 관리: 상품 목록(검색·카테고리 필터), 상품 등록·수정·재고 수정 (`012_admin_products_policy.sql` 실행 필요)
- [x] **관리자(Admin) 3단계** – 회원 관리: 목록(이메일·이름·전화번호 검색), 회원 선택 시 주문 내역·상품·상태, 비밀번호 재설정 안내, 회원 탈퇴 처리 (`013_admin_members_policy.sql` 실행 필요)
- [x] **관리자(Admin) 4단계** – 대시보드(오늘/주간/월간 주문·매출, 상태별 건수, 최근 주문) + 쿠폰 관리(목록·생성·수정). `014_admin_coupons_policy.sql` 실행 필요
- [x] **관리자(Admin) 5단계** – 리뷰 관리: 목록(상품·작성자·평점·내용·작성일), 평점/노출 필터, 숨김·복구. `015_admin_reviews_policy.sql` 실행 필요
- [x] **관리자(Admin) 6단계~** – 공지/이벤트 등
- [x] **재고 표시** – 품절/재고 부족 시 장바구니·구매 비활성화, 주문 시 재고 검증·차감, 관리자 재고 수정(숫자로 입력)
